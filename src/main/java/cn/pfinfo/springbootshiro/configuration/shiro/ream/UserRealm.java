package cn.pfinfo.springbootshiro.configuration.shiro.ream;
...
import cn.pfinfo.springbootshiro.configuration.shiro.authc.UnavalilableAccountException;
...
public class UserRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		try {
			...
			log.error("用户名异常错误",e);
			if(ShiroUtil.getSession().getAttribute("user")!=null){
				ShiroUtil.getSession().removeAttribute("user");
			}
			throw new UnavalilableAccountException("用户名或密码错误");
		}
		if(user == null){
